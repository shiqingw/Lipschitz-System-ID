...
def collect_info(exp_num):
    ...
    estimation_error_file = os.path.join(results_dir, "estimate_error_voronoi.pkl")
    if os.path.exists(estimation_error_file):
        with open(estimation_error_file, "rb") as f:
            ...
            if "global_lipschitz_error" in estimation_error_data:
                ...
                all_info.append(None)
            if "global_lipschitz_time" in estimation_error_data:
                all_info.append(estimation_error_data["global_lipschitz_time"])
            else:
                all_info.append(None)
            if "lipsdp_lipschitz_error" in estimation_error_data:
                ...
                all_info.append(None)
            if "lipsdp_lipschitz_time" in estimation_error_data:
                all_info.append(estimation_error_data["lipsdp_lipschitz_time"])
            else:
                all_info.append(None)
    else:
        all_info.append(None)

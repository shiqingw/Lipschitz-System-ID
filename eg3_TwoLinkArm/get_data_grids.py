...
def get_grid(dataset_num, grid_size):
    ...
    dim_min = np.array([-np.pi/4*3, -np.pi/4*3, -0.1, -0.1]).astype(config.np_dtype)
    dim_max = np.array([np.pi/4*3, np.pi/4*3, 0.1, 0.1]).astype(config.np_dtype)

    assert np.all(x_max >= dim_max)
    assert np.all(x_min <= dim_min)

    # Step 2: Define the grid size
    x1_min, x2_min, x3_min, x4_min = dim_min
    x1_max, x2_max, x3_max, x4_max = dim_max

    # Create the grid
    x1_bins = np.arange(x1_min, x1_max + grid_size, grid_size)
    x2_bins = np.arange(x2_min, x2_max + grid_size, grid_size)
    x3_bins = np.arange(x3_min, x3_max + grid_size, grid_size)
    x4_bins = np.arange(x4_min, x4_max + grid_size, grid_size)
    print(len(x1_bins), len(x2_bins), len(x3_bins), len(x4_bins))

    selected_cells = []  # Temporary list to store the batch of selected cells
    file_counter = 0     # Counter to track the number of files
    grid_counter = 0
    batch_size = int(2500000/(2**x_np.shape[1]*x_np.shape[1])/2)
...
def draw_grid(dataset_num, grid_size):
    ...
    ax.scatter(x_np[:, 0], x_np[:, 1], color='blue', label='2D Points', s=0.5)
    ...
    ax.scatter(x_np[:, 2], x_np[:, 3], color='blue', label='2D Points', s=0.5)

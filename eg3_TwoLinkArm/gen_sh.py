...
def generate_sh_script(filename, exp_nums, device):
    with open(filename, "w") as file:
        for exp_num in exp_nums:
            ...
            command2 = f"python -u eg3_TwoLinkArm/train.py --exp_num {exp_num} --device {device} > eg3_results/{exp_num:03}/output.out\n"
            file.write(command1)
            file.write(command2)

exp_nums = list(range(49, 65)) + list(range(137, 145))

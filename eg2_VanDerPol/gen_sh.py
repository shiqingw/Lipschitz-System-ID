...
def generate_sh_script(filename, exp_nums, device):
    with open(filename, "w") as file:
        for exp_num in exp_nums:
            command1 = f"mkdir eg2_results/{exp_num:03}\n"
            command2 = f"python -u eg2_VanDerPol/train.py --exp_num {exp_num} --device {device} > eg2_results/{exp_num:03}/output.out\n"
            file.write(command1)
            file.write(command2)

exp_nums = list(range(1, 49)) + list(range(73, 137)) + list(range(169, 265))

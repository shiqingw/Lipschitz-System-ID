...
def generate_json_script(filename, entry):
    data = {
            "nominal_system_name": "VanDerPol1_nominal",
            "true_system_name": "VanDerPol1_true",
...
for i in range(len(data)):
    entry = data[i]
    exp_num = exp_nums[i]
    filename = os.path.join(str(Path(__file__).parent.parent), 'eg2_VanDerPol', 'test_settings', f"test_settings_{exp_num:03}.json")

...
result_dir = fullfile(parent_path, 'datasets', 'eg2_VanDerPol', '001');
rng(0);
...
for i=1:size(initial_states,1)
    ...
    x0 = initial_states(i,:)';
    [T_tmp,X_true_tmp] = ode45(@(t,x) eg2_VanDerPol_Dynamics(t,x,mu_vdp,mu_noise,sigma_noise_sqrt_upper),tspan,x0);
    ...
    for j=1:size(T_tmp,1)
        ...
        x_dot_true_tmp = eg2_VanDerPol_Dynamics(t_tmp,x_true_tmp',mu_vdp,zeros(1,n_state),zeros(n_state, n_state))';
    ...
    f = figure('visible','off');
    plot(X_true_tmp(:,1), X_true_tmp(:,2), '--', 'LineWidth', 1);
    hold on;
    plot(X_measured_tmp(:,1), X_measured_tmp(:,2), 'LineWidth', 1);
    hold on;
    plot(X_processed_tmp(:,1), X_processed_tmp(:,2), 'LineWidth', 1);
    legend('(x,y)','(x_n,y_n)','(x_e,y_e)');
    set(gcf,'Position',[100 100 1000 500]);
...
error_ratio = error./X_dot_true*100;
fprintf("==> L2 error: \n");
disp(mean(error.^2, 1));
...
save(fullfile(result_dir,'dataset'),'T','X','X_dot','U', ...
    'mu_vdp','mu_noise','sigma_noise','sigma_noise_sqrt_upper');

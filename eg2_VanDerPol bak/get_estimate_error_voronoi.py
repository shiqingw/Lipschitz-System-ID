...
def get_estimate_error(exp_num, dataset_num, dataset, unique_indices, vor, hull, system_lipschitz, selected_indices):
    ...
    if os.path.exists(global_lipschitz_file):
        ...
        for point_index in range(x_np.shape[0]):
            ...
            ridge_equations = get_surrounding_ridge_equations(vor, point_index)
            vertices = get_vertices_inside_convex_hull(hull, ridge_equations)
            distances = np.linalg.norm(vertices - anchor_point, axis=1)
    ...
    if os.path.exists(lipsdp_lipschitz_file):
        ...
        for point_index in range(x_np.shape[0]):
            ...
            distances = np.linalg.norm(vertices - anchor_point, axis=1)
            max_distance = np.max(distances)
            error = pred_error_np[point_index] + (system_lipschitz + lipsdp_lipschitz) * max_distance
        ...
        total_time = time.time() - start_time
        print("Total time: {} s".format(format_time(total_time)))

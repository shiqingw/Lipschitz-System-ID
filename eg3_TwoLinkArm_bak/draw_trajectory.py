...
def draw(exp_num):
    ...
    for kk in range(num_trajs):
        ...
        for i in range(1, N):
            ...
            control += 100*np.sin(np.pi*t)*torch.ones_like(control)
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(os.path.join(figure_dir, "traj_true_q{:03d}.pdf".format(exp_num)))
    plt.close()
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(os.path.join(figure_dir, "traj_true_dq{:03d}.pdf".format(exp_num)))
    plt.close()
